...
ggplot(df_MeanRatingsVersion, aes(x = Version, y = Rating, label=Rating)) +
  ...
  geom_text(position = 'identity', stat = 'identity', size = 3, vjust = -0.3)+
...
ggplot(df_MeanRatingsDays, aes(x = Day, y = Rating, label = Rating))+
  geom_bar(fill = "#29E58E", stat = "identity")+
  theme_bw() +
  geom_text(position = 'identity', stat = 'identity', size = 0, vjust = -0.4)+
  labs(title="Avg. ratings by day of month") +
